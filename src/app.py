"""
Streamlit dashboard for the stock ranking pipeline.

Changes from original:
- DB connection now uses the shared DATABASE_URL via db.get_engine(),
  consistent with the rest of the pipeline (no more duplicate env-var logic).
- Fixed invalid width='stretch' → use_container_width=True on all
  st.dataframe() and st.plotly_chart() calls.
- Ranking weight sliders now show raw values and their normalised equivalents
  side-by-side so the UI is less confusing.
"""
...
from utils.db import get_engine  # single source of truth for DB connection
...
st.sidebar.caption(
    "Set relative importance for each factor. "
    "Values are automatically normalised to sum to 1."
)

trend_weight      = st.sidebar.slider("Trend Weight",              0.0, 1.0, 0.3, 0.05)
momentum_weight   = st.sidebar.slider("Momentum Weight",           0.0, 1.0, 0.5, 0.05)
volatility_weight = st.sidebar.slider("Volatility Penalty Weight", 0.0, 1.0, 0.2, 0.05)

weight_total = trend_weight + momentum_weight + volatility_weight
if weight_total > 0:
    trend_w      = trend_weight      / weight_total
    momentum_w   = momentum_weight   / weight_total
    volatility_w = volatility_weight / weight_total
else:
    trend_w, momentum_w, volatility_w = 0.3, 0.5, 0.2

st.sidebar.caption(
    f"Normalised → Trend: **{trend_w:.2f}** | "
    f"Momentum: **{momentum_w:.2f}** | "
    f"Volatility: **{volatility_w:.2f}**"
)

# ---------------------------------------------------------------------------
# Apply filters & recompute score with user weights
# ---------------------------------------------------------------------------
...
with tab2:
    ...
    if not detail_df.empty:
        ...
        reasons.append(
            "Trading above MA20" if bool(row["above_ma20"]) else "Trading below MA20"
        )
        reasons.append(
            "Trading above MA50" if bool(row["above_ma50"]) else "Trading below MA50"
        )

        if pd.notna(row["return_5d"]):
            sign = "Positive" if row["return_5d"] > 0 else "Negative"
            reasons.append(f"{sign} 5-day momentum ({row['return_5d']:.2%})")

        if pd.notna(row["return_20d"]):
            sign = "Positive" if row["return_20d"] > 0 else "Negative"
            reasons.append(f"{sign} 20-day momentum ({row['return_20d']:.2%})")
...
with tab3:
    ...
    if history_df.empty:
        ...
        fig.add_trace(go.Scatter(x=history_df["date"], y=history_df["close"], mode="lines", name="Close"))
        fig.add_trace(go.Scatter(x=history_df["date"], y=history_df["ma20"],  mode="lines", name="MA20"))
        fig.add_trace(go.Scatter(x=history_df["date"], y=history_df["ma50"],  mode="lines", name="MA50"))
